...
export async function fetchFirestoreBatch(env, documents, type = null, retried = false) {
    const rawToken = await getAccessToken(env)
    const accessToken = rawToken.access_token
    ...
    if (type) {
        const missing = dataRaw
            .map(item => item.missing?.split("/")?.pop()?.split("_")?.[0] ?? null)
            .filter(Boolean)
...
export async function fetchFIRData(env, callsignList, retried = false) {
    ...
    const data = await fetchFirestoreBatch(env, documents, "FIR", retried)
    return data
}
...
export async function fetchUIRData(env, fssList, retried = false) {
    ...
    const data = await fetchFirestoreBatch(env, documents, "UIR", retried)
    return data
}
